    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (extName === '.json') {
        ...
        return compile(file, opt).then(function(processed) {
          replacedSpace = opt.conservativeCollapse ? ' ' : '';
          content = [
            "function $encodeHtml(str) {", "  return (str + '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\x60/g, '&#96;').replace(/\x27/g, '&#39;').replace(/\x22/g, '&quot;');", "}", "function render($data, $opt) {", "  $data = $data || {};", "  var _$out_= '" + replacedSpace + "';", "  var $print = function(str) {_$out_ += str;};", "  _$out_ += '" + processed.contents.toString().replace(/<\/script>/ig, '</s<%=""%>cript>').replace(/\r\n|\n|\r/g, "\v").replace(/(?:^|%>).*?(?:<%|$)/g, function($0) {
              var res;
