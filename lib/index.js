(function() {
  var EOL, Q, beautify, compile, compileCss, compileLess, compileRiot, compileSass, cssBase64img, fs, getErrorStack, gutil, htmlBase64img, less, path, riot, sass, sus, through, uglify, _;
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (/(\.riot\.html|\.tag)$/.test(originFilePath)) {
        return compileRiot(file, opt).then(function(file) {
          ...
          content = [trace, "define(function(require, exports, module) {", file.contents.toString(), "});"].join(EOL);
          if (opt.beautify) {
            try {
