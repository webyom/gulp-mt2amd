(function() {
  var CSS_MODULE_HELPER, EOL, EXPORTS_REGEXP, PluginError, Q, Vinyl, _, beautify, chalk, compile, compileCss, compileLess, compileSass, crypto, cssBase64img, cssSprite, fixDefineParams, fs, getBodyDeps, getErrorStack, getUnixStylePath, gulpCssSprite, htmlBase64img, less, minifier, path, sass, sus, through, uglify;
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (extName === '.json') {
        ...
        return cssCompiler(file, opt).then(function(file) {
          ...
          content = [trace + (opt.commonjs || opt.es6 ? "" : "define(function(require, exports, module) {"), opt.useExternalCssModuleHelper ? "" : CSS_MODULE_HELPER, "var moduleUri = typeof(module) != 'undefined' && module.uri;", "var expo = yomCssModuleHelper('" + moduleClassName + "', '" + cssContent + "', moduleUri);", opt.es6 ? "__MT2AMD_ES6_EXPORT_DEFAULT__+expo;" : "module.exports = expo;", opt.commonjs || opt.es6 ? "" : "});"].join(EOL);
          if (opt.beautify) {
            try {
