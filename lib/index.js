(function() {
  var EOL, Q, beautify, compile, compileCss, compileLess, fs, gutil, less, path, through, uglify;
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      return less.render(file.contents.toString(), {
        ...
        if (err) {
          ...
          file.contents = new Buffer([trace + '<style type="text/css">', css, '</style>'].join(EOL));
          return resolve(file);
        }
      });
    });
  ...
  compile = function(file, opt, wrap) {
    return Q.Promise(function(resolve, reject) {
      ...
      asyncList = [];
      content = content.replace(/<!--\s*include\s+(['"])([^'"]+)\.(tpl\.html|less|css)\1\s*-->/mg, function(full, quote, incName, ext) {
