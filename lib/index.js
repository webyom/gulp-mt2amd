(function() {
  var EOL, EXPORTS_REGEXP, Q, _, beautify, compile, compileCss, compileLess, compileSass, crypto, cssBase64img, cssSprite, fixDefineParams, fs, getBodyDeps, getErrorStack, getUnixStylePath, gulpCssSprite, gutil, htmlBase64img, less, minifier, path, sass, sus, through, uglify;
  ...
  module.exports.compile = function(file, opt) {
    ...
    return Q.Promise(function(resolve, reject) {
      var content, cssCompiler, e, exportContent, extName, originFilePath, relativePath, trace;
      ...
      if (extName === '.json') {
        ...
        content = [trace + (opt.commonjs || opt.es6 ? "" : "define(function(require, exports, module) {"), opt.es6 ? "export default " + exportContent + ";" : "module.exports = " + exportContent + ";", opt.commonjs || opt.es6 ? "" : "});"].join(EOL);
        file.contents = new Buffer(content);
        ...
        return cssCompiler(file, opt).then(function(file) {
          var cssContent, digest, moduleClassName, originalCssContent;
          if (opt.trace) {
            trace = '/* trace:' + relativePath + ' */' + EOL;
          } else {
            trace = '';
          ...
          digest = crypto.createHash('md5').update(cssContent).digest('hex');
          moduleClassName = digest.slice(0, opt.cssModuleClassNameLength > 0 ? opt.cssModuleClassNameLength : 32);
        ...
        return compile(file, opt).then((function(_this) {
          return function(processed) {
            content = [
              opt.commonjs || opt.es6 ? "" : "define(function(require, exports, module) {", "	function $encodeHtml(str) {", "		return (str + '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\x60/g, '&#96;').replace(/\x27/g, '&#39;').replace(/\x22/g, '&quot;');", "	}", "	function render($data, $opt) {", "		$data = $data || {};", "		var _$out_= '';", "		var $print = function(str) {_$out_ += str;};", "		_$out_ += '" + processed.contents.toString().replace(/<\/script>/ig, '</s<%=""%>cript>').replace(/\r\n|\n|\r/g, "\v").replace(/(?:^|%>).*?(?:<%|$)/g, function($0) {
                return $0.replace(/('|\\)/g, "\\$1").replace(/[\v\t]/g, "").replace(/\s+/g, " ");
              }).replace(/[\v]/g, EOL).replace(/<%==(.*?)%>/g, "' + $encodeHtml($1) + '").replace(/<%=(.*?)%>/g, "' + ($1) + '").replace(/<%(<-)?/g, "';" + EOL + "		").replace(/->(\w+)%>/g, EOL + "		$1 += '").split("%>").join(EOL + "		_$out_ += '") + "';", "		return _$out_;", "	}", opt.es6 ? "__MT2AMD_ES6_EXPORT_DEFAULT__+{render: render};" : "exports.render = render;", opt.commonjs || opt.es6 ? "" : "});"
