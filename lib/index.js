  ...
  compileLess = function(file, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      lessStream.pipe(through.obj(function(file, enc, next) {
        return Q.Promise(function(resolve, reject) {
          if (opt.postcss) {
            return opt.postcss(file, 'less').then(function(file) {
              return resolve({
  ...
  compileSass = function(file, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      sassStream.on('data', function(file) {
        return Q.Promise(function(resolve, reject) {
          if (opt.postcss) {
            return opt.postcss(file, 'scss').then(function(file) {
              return resolve({
                css: file.contents.toString()
              });
            }, reject);
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (extName === '.json') {
        ...
        return compile(file, opt).then(function(processed) {
          var dataInjection, injectionObj;
          replacedSpace = opt.conservativeCollapse ? ' ' : '';
          if (opt.dataInjection) {
            injectionObj = typeof opt.dataInjection === 'string' ? opt.dataInjection : 'window';
            dataInjection = ["function $injectData(name, data) {", "  if (typeof data === 'undefined') {", "    var res = [];", "    for (var p in name) {", "      if (Object.prototype.hasOwnProperty.call(name, p)) {", "        res.push('<script>" + injectionObj + "[\"' + p + '\"] = ' + JSON.stringify(name[p]) + ';</s' + 'cript>');", "      }", "    }", "    return res.join('\\n');", "  }", "  return '<script>" + injectionObj + "[\"' + name + '\"] = ' + JSON.stringify(data) + ';</s' + 'cript>';", "}"].join(EOL);
          }
          content = [
            dataInjection || "", "function $encodeHtml(str) {", "  return (str + '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\x60/g, '&#96;').replace(/\x27/g, '&#39;').replace(/\x22/g, '&quot;');", "}", "function render($data, $opt) {", "  $data = $data || {};", "  var _$out_= '" + replacedSpace + "';", "  var $print = function(str) {_$out_ += str;};", "  _$out_ += '" + processed.contents.toString().replace(/<\/script>/ig, '</s<%=""%>cript>').replace(/\r\n|\n|\r/g, "\v").replace(/(?:^|%>).*?(?:<%|$)/g, function($0) {
              var res;
              res = $0.replace(/('|\\)/g, "\\$1").replace(/\t/g, "").replace(/>\v +/g, ">" + replacedSpace).replace(/\v +</g, replacedSpace + "<").replace(/\v+/g, replacedSpace).replace(/ +/g, " ");
              if (!(/(?:^|%>) *(?:<%|$)/.test(res))) {
                res = res.replace(/(^|%>) /, "$1");
              }
              return res;
            }).replace(/[\v]/g, EOL).replace(/<%==([\s\S]*?)%>/mg, "' + $encodeHtml($1) + '").replace(/<%=([\s\S]*?)%>/mg, "' + ($1) + '").replace(/<%(<-)?/g, "';" + EOL + "  ").replace(/->(\w+)%>/g, EOL + "  $1 += '").split("%>").join(EOL + "  _$out_ += '") + "';", "  return _$out_;", "}"
