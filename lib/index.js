      ...
      if (extName === '.json') {
        ...
        return compile(file, opt).then((function(_this) {
          return function(processed) {
            content = [
              opt.commonjs || opt.esModule ? "" : "define(function(require, exports, module) {", "	function $encodeHtml(str) {", "		return (str + '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\x60/g, '&#96;').replace(/\x27/g, '&#39;').replace(/\x22/g, '&quot;');", "	}", "	function render($data, $opt) {", "		$data = $data || {};", "		var _$out_= '';", "		var $print = function(str) {_$out_ += str;};", "		_$out_ += '" + processed.contents.toString().replace(/<\/script>/ig, '</s<%=""%>cript>').replace(/\r\n|\n|\r/g, "\v").replace(/(?:^|%>).*?(?:<%|$)/g, function($0) {
                return $0.replace(/('|\\)/g, "\\$1").replace(/[\v\t]/g, "").replace(/\s+/g, " ");
              }).replace(/[\v]/g, EOL).replace(/<%==(.*?)%>/g, "' + $encodeHtml($1) + '").replace(/<%=(.*?)%>/g, "' + ($1) + '").replace(/<%(<-)?/g, "';" + EOL + "		").replace(/->(\w+)%>/g, EOL + "		$1 += '").split("%>").join(EOL + "		_$out_ += '") + "';", "		return _$out_;", "	}", opt.esModule ? "__MT2AMD_ES_MODULE_EXPORT_DEFAULT__+{render: render};" : "exports.render = render;", opt.commonjs || opt.esModule ? "" : "});"
