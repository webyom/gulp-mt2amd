(function() {
  var EOL, EXPORTS_REGEXP, Q, RIOT_EXT_REGEXP, _, beautify, compile, compileCss, compileLess, compileRiot, compileSass, cssBase64img, cssSprite, fixDefineParams, fs, getBodyDeps, getErrorStack, getUnixStylePath, gulpCssSprite, gutil, htmlBase64img, less, path, riot, sass, sus, through, uglify;
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (RIOT_EXT_REGEXP.test(originFilePath)) {
        ...
        return cssCompiler(file, opt).then(function(file) {
          var content, e, error, trace;
          if (opt.trace) {
            trace = '/* trace:' + path.relative(process.cwd(), originFilePath) + ' */' + EOL;
          } else {
            trace = '';
          }
          content = [opt.commonjs ? "" : "define(function(require, exports, module) {", "	exports.render = function() {", "		var _$out_= [];", trace + "		_$out_.push('" + file._cssContents.toString().replace(/\r\n|\n|\r/g, '').replace(/\s+/g, ' ') + "');", "		return _$out_.join('');", "	};", opt.commonjs ? "" : "});"].join(EOL);
          if (opt.beautify) {
            try {
              content = beautify(content, opt.beautify);
            } catch (error) {
        ...
        return compile(file, opt).then((function(_this) {
          return function(processed) {
            ...
            content = [
              ...
              }).replace(/[\v]/g, EOL).replace(/<%==(.*?)%>/g, "', $encodeHtml($1), '").replace(/<%=(.*?)%>/g, "', $1, '").replace(/<%(<-)?/g, "');" + EOL + "		").replace(/->(\w+)%>/g, EOL + "		$1.push('").split("%>").join(EOL + "		_$out_.push('") + "');", "		return _$out_.join('');", "	};", opt.commonjs ? "" : "});"
