(function() {
  var EOL, Q, beautify, compile, compileCss, compileLess, compileSass, fs, getErrorStack, gutil, less, path, sass, through, uglify;
  ...
  compileCss = function(file, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      file.contents = new Buffer([trace + '<style type="text/css">', opt.postcss ? opt.postcss(file) : file.contents.toString(), '</style>'].join(EOL));
      return resolve(file);
    });
  };
