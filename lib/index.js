      ...
      if (extName === '.json') {
        ...
        return compile(file, opt).then(function(processed) {
          ...
          if (opt.dataInjection) {
            dataInjection = ["function $injectData(name, data) {", "  return '<script>" + (typeof opt.dataInjection === 'string' ? opt.dataInjection : 'window') + "[\"' + name + '\"] = ' + JSON.stringify(data) + ';</s' + 'cript>';", "}"].join(EOL);
          }
          content = [
